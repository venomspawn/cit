...
module CIT
  ...
  # Value of `RACK_ENV` environment variable with the mean of development
  # environment
  DEVELOPMENT = 'development'

  # Value of `RACK_ENV` environment variable with the mean of test environment
  TEST = 'test'

  # Value of `RACK_ENV` environment variable with the mean of production
  # environment
  ...
  # constant if the value of `RACK_ENV` environment variable is absent or not
  # supported
  # @return [String]
  #   result value
  def self.env
  ...
  # Returns if application is in development environment or not
  # @return [Boolean]
  #   if application is in development environment or not
  def self.development?
    env == DEVELOPMENT
  end

  # Returns if application is in test environment or not
  # @return [Boolean]
  #   if application is in test environment or not
  def self.test?
    env == TEST
  end

  # Returns if application is in production environment or not
  # @return [Boolean]
  #   if application is in production environment or not
  def self.production?
    env == PRODUCTION
  end
  ...
  # List which is used to skip exceptions of `StandardError` class
  ...
  #       used (which means no exception would be muted).
  ...
  # Loads Ruby source file by provided path. Mutes exceptions raised during the
  # load if their class ancestors belong to the provided set.
  # @param [#to_s] path
  #   source file path
  # @param [#include?] skip
